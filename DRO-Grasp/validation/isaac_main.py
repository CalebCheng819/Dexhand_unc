...
if __name__ == '__main__':
    ...
    if args.mode == 'filter':
        ...
        if args.ret_file is not None and len(str(args.ret_file)) > 0:
            ret_path = args.ret_file
            ret_dir = os.path.dirname(ret_path)
            if ret_dir:
                os.makedirs(ret_dir, exist_ok=True)
        else:
            os.makedirs(os.path.join(ROOT_DIR, 'tmp'), exist_ok=True)
            ret_path = os.path.join(ROOT_DIR, f'tmp/isaac_main_ret_{args.gpu}.pt')
        torch.save(save_data, ret_path)
