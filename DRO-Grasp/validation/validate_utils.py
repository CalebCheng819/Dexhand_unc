...
def validate_isaac(robot_name, object_name, q_batch, gpu: int = 0):
    ...
    # Use per-call unique tmp filenames so parallel eval jobs do not clobber each other.
    unique_tag = f"{gpu}_{os.getpid()}_{int(time.time() * 1e6)}_{uuid.uuid4().hex[:8]}"
    q_file_path = str(os.path.join(tmp_dir, f'q_list_validate_{unique_tag}.pt'))
    ret_file_path = str(os.path.join(tmp_dir, f'isaac_main_ret_{unique_tag}.pt'))
    ...
    try:
        ...
        raise RuntimeError("Isaac subprocess did not produce return file; see logs above.") from e
    finally:
